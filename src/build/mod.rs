...
pub(crate) fn render_table_column(column: &Column) -> String {
    ...
    if let Some(generated) = &column.generated {
        if let Some(expression) = &generated.expression {
            ...
            || generated.sequence_behavior.is_some()
        {
            // a pulled identity has no `sequence`, which older project
            // files use for a separately managed sequence; the
            // behavior alone says the column is an identity
            sql.push(render_identity(generated));
        }
    }
    sql.join(" ")
}

/// `GENERATED <behavior> AS IDENTITY [(options)]`, shared by the column
/// definition and by deploy's `ALTER COLUMN ... ADD`
pub(crate) fn render_identity(
    generated: &crate::models::ColumnGenerated,
) -> String {
    let mut sql = format!(
        "GENERATED {} AS IDENTITY",
        generated.sequence_behavior.clone().unwrap_or_default()
    );
    if let Some(options) = &generated.sequence_options {
        sql.push_str(&format!(" ({})", render_sequence_options(options)));
    }
    sql
}

/// An identity column's sequence options, in the order pg_dump writes
/// them. Only the options the model holds are written; each absent one
/// is PostgreSQL's default, so the server fills it in identically.
fn render_sequence_options(
    options: &crate::models::SequenceOptions,
) -> String {
    let mut parts = Vec::new();
    if let Some(name) = &options.name {
        parts.push(format!("SEQUENCE NAME {name}"));
    }
    if let Some(start) = options.start_with {
        parts.push(format!("START WITH {start}"));
    }
    if let Some(by) = options.increment_by {
        parts.push(format!("INCREMENT BY {by}"));
    }
...
mod tests {
    ...
    #[test]
    fn renders_identity_sequence_options() {
        use crate::models::{ColumnGenerated, SequenceOptions};

        let identity = |options| {
            let mut c = column("id", "integer", true);
            c.generated = Some(ColumnGenerated {
                expression: None,
                kind: None,
                sequence: None,
                sequence_behavior: Some("BY DEFAULT".into()),
        ...
        // a pulled identity has no legacy `sequence`, and must still
        // render as an identity rather than a plain column
        assert!(
            identity(None).contains(
                "id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY )"
            ),
            "{}",
            identity(None)
        );
        let tuned = identity(Some(SequenceOptions {
            name: Some("test.order_ids".into()),
            start_with: Some(100),
            increment_by: Some(5),
            min_value: None,
            max_value: Some(900),
        ...
        assert!(
            tuned.contains(
                "GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME \
                 test.order_ids START WITH 100 INCREMENT BY 5 MAXVALUE 900 \
                 CACHE 20 CYCLE)"
            ),
            "{tuned}"
        );
