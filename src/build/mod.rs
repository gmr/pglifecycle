...
//!     entry to exclude.
//! 20. An identity column that names a `sequence` but no
//!     `sequence_behavior` renders `GENERATED BY DEFAULT AS IDENTITY`.
...
pub(crate) fn render_identity(
    ...
    // an older project file can name a sequence and no behavior;
    // PostgreSQL requires one, so use its own default (deviation 20)
    let mut sql = format!(
    ...
    // `sequence_options: {}` holds no option, and an empty `()` is a
    // syntax error
    let options = generated
        .sequence_options
        .as_ref()
        .map(render_sequence_options)
        .unwrap_or_default();
    if !options.is_empty() {
...
mod tests {
    ...
    fn renders_identity_sequence_options() {
        ...
        // options that hold no clause render no empty `()`
        let empty = identity(Some(SequenceOptions {
            cycle: Some(false),
            ..SequenceOptions::default()
        }));
        assert!(
            empty.contains("GENERATED BY DEFAULT AS IDENTITY )"),
            "{empty}"
        );
        // an older file names a sequence and no behavior (deviation 20)
        assert_eq!(
            render_identity(&ColumnGenerated {
                expression: None,
                kind: None,
                sequence: Some("orders_id".into()),
