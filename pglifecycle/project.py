...
class Project:
    ...
    def __init__(
        ...
        self.inventory: list[models.Item] = []
    ...
    def __repr__(self) -> str:
        return f'<Project path="{self.path!s}">'
    ...
    def create(
        self, force: bool = False, gitkeep: bool = True
    ) -> typing.NoReturn:
    ...
    def load(self) -> Project:
        ...
        if self._load_errors:
            LOGGER.critical(
                'Project load failed with %i errors', self._load_errors
            )
    ...
    def _apply_cached_dependencies(self):
        for dep in self._cached_dependencies:
            ...
            if not item:
                raise RuntimeError(
                    f'Failed to find {dep.desc} {dep.namespace}.{dep.tag} for {dep.parent_desc} {dep.parent_namespace}.{dep.parent_tag}'
                )
            if not parent:
                raise RuntimeError(
                    f'Failed to find parent {dep.parent_desc} {dep.parent_namespace}.{dep.parent_tag} for {dep.desc} {dep.namespace}.{dep.tag}'
                )
            item.dependencies.add(parent.id)

    def _cache_and_remove_dependencies(
        self, desc: str, definition: dict
    ) -> typing.NoReturn:
    ...
    def _create_directories(
        self, exist_ok: bool = False, gitkeep: bool = True
    ) -> typing.NoReturn:
    ...
    def _load_table_definition(self, defn: dict) -> models.Table:
        if 'columns' in defn:
            defn['columns'] = [
                self._load_column_definition(c) for c in defn['columns']
            ]
        if 'indexes' in defn:
            defn['indexes'] = [
                self._load_index_definition(i) for i in defn['indexes']
            ]
        ...
        if 'partitions' in defn:
            defn['partitions'] = [
                models.TablePartition(**p) for p in defn['partitions']
            ]
        if 'foreign_keys' in defn:
            defn['foreign_keys'] = [
                self._load_fk_definition(fk) for fk in defn['foreign_keys']
            ]
    ...
    def _load_text_search_definition(defn: dict) -> typing.NoReturn:
        ...
        if defn.get('configurations'):
            configs = [
                models.TextSearchConfig(**c) for c in defn['configurations']
            ]
        if defn.get('dictionaries'):
            dicts = [models.TextSearchDict(**d) for d in defn['dictionaries']]
        if defn.get('parsers'):
            parsers = [models.TextSearchParser(**p) for p in defn['parsers']]
        if defn.get('templates'):
            templates = [
                models.TextSearchTemplate(**t) for t in defn['templates']
            ]
    ...
    def _lookup_item(
        self, desc: str, namespace: str | None, tag: str
    ) -> models.Item | None:
        for item in self.inventory:
            if (
                desc in self._SCHEMALESS
                and item.desc == desc
    ...
    def _read_object_files(
        self, desc: str, model: dataclasses.dataclass
    ) -> typing.NoReturn:
    ...
    def _read_objects_files(
        self, desc: str, model: dataclasses.dataclass
    ) -> typing.NoReturn:
        LOGGER.debug('Reading %s objects', desc)
        key = next(k for k, v in constants.OBJ_KEYS.items() if v == desc)
        for defn in self._iterate_files(desc):
    ...
    def _read_user_mapping_files(self) -> typing.NoReturn:
        LOGGER.debug(
            'Reading %s', constants.PATHS[constants.USER_MAPPING].name.upper()
        )
        for defn in self._iterate_files(constants.USER_MAPPING):
            ...
            defn['servers'] = [
                models.UserMappingServer(**s) for s in defn['servers']
            ]
